...
describe('sunTimes', () => {
  it('matches the sunrise and sunset NWS reported for 2026-08-10', () => {
    const { sunrise, sunset } = sunTimes(new Date('2026-08-10T12:00:00Z'), spot)
...
describe('isDaylight', () => {
  it('accepts midday and rejects the small hours', () => {
    expect(isDaylight('2026-08-10T16:00:00Z', spot)).toBe(true)  // noon ET
    expect(isDaylight('2026-08-10T07:00:00Z', spot)).toBe(false) // 3am ET
  })

  it('rejects an hour just before sunrise and accepts one just after', () => {
    expect(isDaylight('2026-08-10T09:00:00Z', spot)).toBe(false) // 5am ET
    expect(isDaylight('2026-08-10T11:00:00Z', spot)).toBe(true)  // 7am ET
  })
})
